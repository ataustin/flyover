...
This vignette will introduce alternative plotting functions for step 2.  While there are some built-in `flyover` plotting functions ready to use, there will be times that you need something customized to your work.  You can write your own plotting function that can still be used in a `flyover` workflow.
...
The built-in plotting functions that ship with this package are very simple wrappers for `ggplot2` plots.  For example, this is the body of the histogram function:
...
Let's write a custom plot function to show how it might work.  Let's say we wish to compare distributions not through the use of histograms, but with empirical cumulative distribution functions (ECDFs).  After some research on the `ggplot2` [website](https://ggplot2.tidyverse.org/index.html), we find the right syntax.
