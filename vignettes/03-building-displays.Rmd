...
The data used in this example is the same as in the previous vignette ("Creating plots").
...
The following code would render a display in an R Mardown document (see the tips below for rendering displays).  Notice that the display is interactive, allowing you to page through multiple plots, change the grid size, add and remove metadata labels, filter plots by metadata features, and sort them on the same features.  Sorting and filtering the plots is a powerful way to cut out noise and focus on the plots where the most important changes are taking place.
...
This package makes it easy to break down data sets in different ways and compare distributions.  For example, if we decide we are interested in numeric distributions across different kinds of fruit, we could do the following:
...
There are three ways you could consider rendering and viewing the display:

1. point a browser directly at the output directory (_e.g._ if you are serving files from localhost);
2. render the display from an R Mardown, ensuring that the `build_display` call is the last line of a code chunk and the output directory is a relative file path;
3. embed the display in an iframe, pointing `src` to the display output.
